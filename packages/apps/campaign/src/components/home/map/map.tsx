...
const Entity = memo(function Entity(props: { entity: Types.Campaign.MapItem, getMapPosition: (pos: { x: number, y: number }) => MapPosition }) {
    ...
    return position == null ? null : <Marker
...
export function Map() {
    ...
    return <div className="w-full h-full flex-1 flex">
        <MaplibreMap
            {...viewState}
        ...
        >
            {entities.map((entity) => <Entity key={entity.id} entity={entity} getMapPosition={getMapPosition} />)}
        </MaplibreMap>
    </div>;
}
